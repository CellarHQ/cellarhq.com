/*
 * This file is generated by jOOQ.
 */
package com.cellarhq.generated.tables.records;


import com.cellarhq.generated.tables.Glassware;

import java.sql.Timestamp;

import javax.annotation.Generated;
import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;
import javax.persistence.Id;
import javax.persistence.Index;
import javax.persistence.Table;
import javax.persistence.UniqueConstraint;

import org.jooq.Field;
import org.jooq.JSON;
import org.jooq.Record1;
import org.jooq.Record10;
import org.jooq.Row10;
import org.jooq.impl.UpdatableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.12.3"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
@Entity
@Table(name = "glassware", schema = "public", uniqueConstraints = {
    @UniqueConstraint(name = "pk_glassware", columnNames = {"id"})
}, indexes = {
    @Index(name = "idx_glassware_brewery_db_id", columnList = "brewery_db_id ASC"),
    @Index(name = "pk_glassware", unique = true, columnList = "id ASC")
})
public class GlasswareRecord extends UpdatableRecordImpl<GlasswareRecord> implements Record10<Long, Integer, String, String, Boolean, String, Timestamp, Timestamp, Timestamp, JSON> {

    private static final long serialVersionUID = 823145616;

    /**
     * Setter for <code>public.glassware.id</code>.
     */
    public void setId(Long value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.glassware.id</code>.
     */
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "id", nullable = false, precision = 64)
    public Long getId() {
        return (Long) get(0);
    }

    /**
     * Setter for <code>public.glassware.version</code>.
     */
    public void setVersion(Integer value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.glassware.version</code>.
     */
    @Column(name = "version", nullable = false, precision = 32)
    public Integer getVersion() {
        return (Integer) get(1);
    }

    /**
     * Setter for <code>public.glassware.name</code>.
     */
    public void setName(String value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.glassware.name</code>.
     */
    @Column(name = "name", nullable = false, length = 100)
    public String getName() {
        return (String) get(2);
    }

    /**
     * Setter for <code>public.glassware.description</code>.
     */
    public void setDescription(String value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.glassware.description</code>.
     */
    @Column(name = "description")
    public String getDescription() {
        return (String) get(3);
    }

    /**
     * Setter for <code>public.glassware.searchable</code>.
     */
    public void setSearchable(Boolean value) {
        set(4, value);
    }

    /**
     * Getter for <code>public.glassware.searchable</code>.
     */
    @Column(name = "searchable", nullable = false)
    public Boolean getSearchable() {
        return (Boolean) get(4);
    }

    /**
     * Setter for <code>public.glassware.brewery_db_id</code>.
     */
    public void setBreweryDbId(String value) {
        set(5, value);
    }

    /**
     * Getter for <code>public.glassware.brewery_db_id</code>.
     */
    @Column(name = "brewery_db_id", length = 64)
    public String getBreweryDbId() {
        return (String) get(5);
    }

    /**
     * Setter for <code>public.glassware.brewery_db_last_updated</code>.
     */
    public void setBreweryDbLastUpdated(Timestamp value) {
        set(6, value);
    }

    /**
     * Getter for <code>public.glassware.brewery_db_last_updated</code>.
     */
    @Column(name = "brewery_db_last_updated")
    public Timestamp getBreweryDbLastUpdated() {
        return (Timestamp) get(6);
    }

    /**
     * Setter for <code>public.glassware.created_date</code>.
     */
    public void setCreatedDate(Timestamp value) {
        set(7, value);
    }

    /**
     * Getter for <code>public.glassware.created_date</code>.
     */
    @Column(name = "created_date", nullable = false)
    public Timestamp getCreatedDate() {
        return (Timestamp) get(7);
    }

    /**
     * Setter for <code>public.glassware.modified_date</code>.
     */
    public void setModifiedDate(Timestamp value) {
        set(8, value);
    }

    /**
     * Getter for <code>public.glassware.modified_date</code>.
     */
    @Column(name = "modified_date", nullable = false)
    public Timestamp getModifiedDate() {
        return (Timestamp) get(8);
    }

    /**
     * Setter for <code>public.glassware.data</code>.
     */
    public void setData(JSON value) {
        set(9, value);
    }

    /**
     * Getter for <code>public.glassware.data</code>.
     */
    @Column(name = "data")
    public JSON getData() {
        return (JSON) get(9);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    public Record1<Long> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record10 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row10<Long, Integer, String, String, Boolean, String, Timestamp, Timestamp, Timestamp, JSON> fieldsRow() {
        return (Row10) super.fieldsRow();
    }

    @Override
    public Row10<Long, Integer, String, String, Boolean, String, Timestamp, Timestamp, Timestamp, JSON> valuesRow() {
        return (Row10) super.valuesRow();
    }

    @Override
    public Field<Long> field1() {
        return Glassware.GLASSWARE.ID;
    }

    @Override
    public Field<Integer> field2() {
        return Glassware.GLASSWARE.VERSION;
    }

    @Override
    public Field<String> field3() {
        return Glassware.GLASSWARE.NAME;
    }

    @Override
    public Field<String> field4() {
        return Glassware.GLASSWARE.DESCRIPTION;
    }

    @Override
    public Field<Boolean> field5() {
        return Glassware.GLASSWARE.SEARCHABLE;
    }

    @Override
    public Field<String> field6() {
        return Glassware.GLASSWARE.BREWERY_DB_ID;
    }

    @Override
    public Field<Timestamp> field7() {
        return Glassware.GLASSWARE.BREWERY_DB_LAST_UPDATED;
    }

    @Override
    public Field<Timestamp> field8() {
        return Glassware.GLASSWARE.CREATED_DATE;
    }

    @Override
    public Field<Timestamp> field9() {
        return Glassware.GLASSWARE.MODIFIED_DATE;
    }

    @Override
    public Field<JSON> field10() {
        return Glassware.GLASSWARE.DATA;
    }

    @Override
    public Long component1() {
        return getId();
    }

    @Override
    public Integer component2() {
        return getVersion();
    }

    @Override
    public String component3() {
        return getName();
    }

    @Override
    public String component4() {
        return getDescription();
    }

    @Override
    public Boolean component5() {
        return getSearchable();
    }

    @Override
    public String component6() {
        return getBreweryDbId();
    }

    @Override
    public Timestamp component7() {
        return getBreweryDbLastUpdated();
    }

    @Override
    public Timestamp component8() {
        return getCreatedDate();
    }

    @Override
    public Timestamp component9() {
        return getModifiedDate();
    }

    @Override
    public JSON component10() {
        return getData();
    }

    @Override
    public Long value1() {
        return getId();
    }

    @Override
    public Integer value2() {
        return getVersion();
    }

    @Override
    public String value3() {
        return getName();
    }

    @Override
    public String value4() {
        return getDescription();
    }

    @Override
    public Boolean value5() {
        return getSearchable();
    }

    @Override
    public String value6() {
        return getBreweryDbId();
    }

    @Override
    public Timestamp value7() {
        return getBreweryDbLastUpdated();
    }

    @Override
    public Timestamp value8() {
        return getCreatedDate();
    }

    @Override
    public Timestamp value9() {
        return getModifiedDate();
    }

    @Override
    public JSON value10() {
        return getData();
    }

    @Override
    public GlasswareRecord value1(Long value) {
        setId(value);
        return this;
    }

    @Override
    public GlasswareRecord value2(Integer value) {
        setVersion(value);
        return this;
    }

    @Override
    public GlasswareRecord value3(String value) {
        setName(value);
        return this;
    }

    @Override
    public GlasswareRecord value4(String value) {
        setDescription(value);
        return this;
    }

    @Override
    public GlasswareRecord value5(Boolean value) {
        setSearchable(value);
        return this;
    }

    @Override
    public GlasswareRecord value6(String value) {
        setBreweryDbId(value);
        return this;
    }

    @Override
    public GlasswareRecord value7(Timestamp value) {
        setBreweryDbLastUpdated(value);
        return this;
    }

    @Override
    public GlasswareRecord value8(Timestamp value) {
        setCreatedDate(value);
        return this;
    }

    @Override
    public GlasswareRecord value9(Timestamp value) {
        setModifiedDate(value);
        return this;
    }

    @Override
    public GlasswareRecord value10(JSON value) {
        setData(value);
        return this;
    }

    @Override
    public GlasswareRecord values(Long value1, Integer value2, String value3, String value4, Boolean value5, String value6, Timestamp value7, Timestamp value8, Timestamp value9, JSON value10) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        value10(value10);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached GlasswareRecord
     */
    public GlasswareRecord() {
        super(Glassware.GLASSWARE);
    }

    /**
     * Create a detached, initialised GlasswareRecord
     */
    public GlasswareRecord(Long id, Integer version, String name, String description, Boolean searchable, String breweryDbId, Timestamp breweryDbLastUpdated, Timestamp createdDate, Timestamp modifiedDate, JSON data) {
        super(Glassware.GLASSWARE);

        set(0, id);
        set(1, version);
        set(2, name);
        set(3, description);
        set(4, searchable);
        set(5, breweryDbId);
        set(6, breweryDbLastUpdated);
        set(7, createdDate);
        set(8, modifiedDate);
        set(9, data);
    }
}
